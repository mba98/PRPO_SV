...
function ResubmitIcon({ className = 'h-4 w-4' }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
...
export default function ApriDetailView({ id }) {
  ...
  async function handleResubmit() {
    ...
    if (json.success) {
      ...
      setActionError(json.message || apriI18n.resubmitFailed);
    }
  }
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canResubmit && (
              <Button
                ...
                className="w-auto shrink-0 rounded-xl px-4 py-2 focus-visible:ring-2 focus-visible:ring-destructive/40"
              >
                {!resubmitting ? <ResubmitIcon className="h-4 w-4 shrink-0" /> : null}
                {resubmitting ? apriI18n.resubmitting : apriI18n.resubmit}
              </Button>
            )}
            {canRetry && (
