...
function makePoDoc(overrides = {}) {
  const status = overrides.status ?? 'Pending Project Manager Approval';
  const line = {
    _id: 'lineid1',
    itemCode: 'ITEM1',
    quantity: 2,
...
describe('updatePurchaseOrder', () => {
  ...
  it('allows editing while Pending Project Manager Approval', async () => {
    const po = makePoDoc();
  ...
  it('rejects editing when Created in SAP', async () => {
    const po = makePoDoc({ status: 'Created in SAP', sapPODocEntry: 100 });
    mocks.findById.mockResolvedValue(po);
