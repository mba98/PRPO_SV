...
const FOOTER = 'Procurement Portal · SPC';
...
function wrapHtml({ title, greetingLine, bodyHtml, documentType, documentId, ctaLabel }) {
  return `<!DOCTYPE html>
<html>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid #e2e8f0;border-radius:8px;">
        <tr><td style="padding:28px 32px;">
          ...
          ${bodyHtml}
          ${documentType && documentId ? ctaButton(documentType, documentId, ctaLabel) : ''}
...
export function buildWorkflowEmail(templateKey, data = {}) {
  ...
  const builders = {
    ...
    'pr.pm.approved': () => {
      const docType = 'PR';
      const docId = data.documentId;
      const subject = `PR ${prNum} approved — pending next step`;
      const text = `${greetText}\n\nPurchase Request ${prNum} was approved by the project manager.\n\nOpen: ${buildDocumentUrl(docType, docId)}\n\n${FOOTER}`;
      const html = wrapHtml({
        title: subject,
        greetingLine: greet,
        documentType: docType,
        documentId: docId,
        ctaLabel: CTA_LABELS.PR,
        bodyHtml: `<p style="margin:0 0 12px;font-size:15px;">Purchase Request <strong>${escapeHtml(prNum)}</strong> was approved by the project manager.</p>
          ${detailsTable([detailRow('Document', prNum)])}`,
      });
      return { subject, html, text };
    },
    ...
    'pr.rejected': () => {
      ...
      const text = `${greetText}\n\nPurchase Request ${prNum} was rejected.${comment ? `\nComment: ${comment}` : ''}\n\nOpen: ${buildDocumentUrl(docType, docId)}\n\n${FOOTER}`;
      const html = wrapHtml({
        title: subject,
        greetingLine: greet,
        documentType: docType,
        documentId: docId,
        ctaLabel: CTA_LABELS.PR,
        bodyHtml: `<p style="margin:0 0 12px;font-size:15px;">Purchase Request <strong>${escapeHtml(prNum)}</strong> was <strong style="color:#b91c1c;">rejected</strong>.</p>
          ${detailsTable([
            detailRow('Document', prNum),
    ...
    'po.finance.approved': () => {
      ...
      const text = `${greetText}\n\nPurchase Order ${poNum} was fully approved and will be created in SAP.\n\nOpen: ${buildDocumentUrl(docType, docId)}\n\n${FOOTER}`;
      const html = wrapHtml({
        title: subject,
        greetingLine: greet,
        documentType: docType,
        documentId: docId,
        ctaLabel: CTA_LABELS.PO,
        bodyHtml: `<p style="margin:0 0 12px;font-size:15px;">Purchase Order <strong>${escapeHtml(poNum)}</strong> was <strong>fully approved</strong> and will be created in SAP.</p>
          ${detailsTable([
            detailRow('Document', poNum),
