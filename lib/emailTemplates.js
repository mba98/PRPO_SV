...
export function buildWorkflowEmail(templateKey, data = {}) {
  ...
  const builders = {
    ...
    'po.pm.approved': () => {
      const docType = 'PO';
      const docId = data.documentId;
      const subject = `PO ${poNum} approved — pending Operation Manager`;
      const text = `${greetText}\n\nPurchase Order ${poNum} was approved by the project manager and now requires operation manager approval.\n\nOpen: ${buildDocumentUrl(docType, docId)}\n\n${FOOTER}`;
      const html = wrapHtml({
        title: subject,
        greetingLine: greet,
        documentType: docType,
        documentId: docId,
        ctaLabel: CTA_LABELS.PO,
        bodyHtml: `<p style="margin:0 0 12px;font-size:15px;">Purchase Order <strong>${escapeHtml(poNum)}</strong> was approved by the <strong>project manager</strong> and now requires <strong>operation manager approval</strong>.</p>
          ${detailsTable([
            detailRow('Document', poNum),
            detailRow('Status', 'Pending Operation Manager Approval'),
          ])}`,
      });
      return { subject, html, text };
    },
    ...
    'po.om.approved': () => {
      const docType = 'PO';
      const docId = data.documentId;
      const subject = `PO ${poNum} approved by Operation Manager — pending Finance`;
      const text = `${greetText}\n\nPurchase Order ${poNum} was approved by the operation manager and now requires finance approval.\n\nOpen: ${buildDocumentUrl(docType, docId)}\n\n${FOOTER}`;
      const html = wrapHtml({
        title: subject,
        greetingLine: greet,
        documentType: docType,
        documentId: docId,
        ctaLabel: CTA_LABELS.PO,
        bodyHtml: `<p style="margin:0 0 12px;font-size:15px;">Purchase Order <strong>${escapeHtml(poNum)}</strong> was approved by the <strong>operation manager</strong> and now requires <strong>finance approval</strong>.</p>
          ${detailsTable([
            detailRow('Document', poNum),
