...
import {
  sendLocalPurchaseSubmitEmail,
  sendLocalPurchaseWorkflowEmail,
} from '@/lib/lpEmailNotify.js';
import { buildLpEmailContext } from '@/lib/emailContext.js';
import { enrichLpEmailContextFromHistory } from '@/lib/lpEmailContext.js';
import { resolveLpApprovalHistoryActorEmails } from '@/lib/lpEmailRecipients.js';
...
import {
  ...
  LP_FINANCE_APPROVAL_PERMISSION,
  LP_VIEW_ALL_PERMISSION,
  logLocalPurchaseFinanceAccessDiagnostics,
...
import { normalizeLpCurrency } from '@/lib/lpMoney.js';
...
function normalizeCurrency(value) {
  const raw = value == null || value === '' ? 'IQD' : String(value).trim().toUpperCase();
  if (!['IQD', 'USD'].includes(raw)) {
    const err = new Error('Currency must be IQD or USD');
    err.code = 'VALIDATION_ERROR';
    throw err;
  }
  return raw;
...
export function sanitizeLocalPurchase(doc) {
  if (!doc) return null;
  const normalized = normalizeLpDocumentState(doc);
  const id = normalized._id?.toString() || normalized.id;
  const lines = (normalized.lines || []).map((line) => sanitizeLpLine(line));
  return {
    id,
    portalLPNumber: normalized.portalLPNumber,
    documentDate: normalized.documentDate,
    currency: normalizeLpCurrency(normalized.currency),
    budget: Number(normalized.budget ?? 0),
    remarks: normalized.remarks,
    lines,
    documentTotal: normalized.documentTotal,
    status: normalizeLpStatus(normalized.status),
    currentApprovalStep: normalized.currentApprovalStep ?? 0,
    createdBy: normalized.createdBy?._id?.toString() || normalized.createdBy?.toString(),
    createdByName: normalized.createdBy?.name || normalized.createdByName,
...
async function buildLpPendingApprovalFilter(user) {
  ...
  for (const step of steps) {
    if (
      permissions.includes(step.requiredPermission) &&
      matchesApproverRole(user, step, DOC_TYPE)
    ) {
      or.push({
...
async function buildLocalPurchaseDetailPayload(doc, user, steps) {
  const normalizedDoc = normalizeLpDocumentState(doc);
  const workflowSteps = await loadLpWorkflow(normalizedDoc, user, steps);
  const canEdit = userCanEditLocalPurchase(user, normalizedDoc);
  const canCancel = userCanCancelLocalPurchase(user, normalizedDoc);
  const canSubmit =
    canEdit &&
    (lpStatusesEqual(normalizedDoc.status, LP_STATUS.DRAFT) ||
      lpStatusesEqual(normalizedDoc.status, LP_STATUS.REJECTED));
  const canResubmit = canSubmit && lpStatusesEqual(normalizedDoc.status, LP_STATUS.REJECTED);

  const approvalAccess = buildDocumentApprovalAccess({
    documentType: DOC_TYPE,
    document: sanitizeLocalPurchase(normalizedDoc),
    user,
    approvalSteps: steps,
  });

  return {
    ...sanitizeLocalPurchase(normalizedDoc),
    workflowSteps,
    ...approvalAccess,
    canEdit,
    canCancel,
...
export async function getLocalPurchaseById(id, user) {
  return perfAsync(`getLocalPurchaseById ${id}`, async () => {
    ...
    if (!doc) return null;
    if (!userCanViewLocalPurchase(user, doc, steps)) {
      if (process.env.NODE_ENV === 'development') {
        logLocalPurchaseFinanceAccessDiagnostics(user, doc, steps, { canView: false });
      }
      const err = new Error('Forbidden');
      err.code = 'FORBIDDEN';
      throw err;
...
async function loadLpForUpdate(id, user) {
  ...
  const steps = await getApprovalSteps(DOC_TYPE);
  if (!userCanViewLocalPurchase(user, doc.toObject(), steps)) {
    const err = new Error('Forbidden');
    err.code = 'FORBIDDEN';
    throw err;
  }
...
export async function updateLocalPurchase(id, data, user) {
  ...
  const existing = await LocalPurchase.findById(id);
  if (!existing) {
    const err = new Error('Local Purchase not found');
    err.code = 'NOT_FOUND';
    throw err;
  }
  if (!userCanEditLocalPurchase(user, existing.toObject())) {
  ...
  const updated = await LocalPurchase.findOneAndUpdate(
    { _id: id, __v: data.__v },
    {
      $set: {
        ...
        lines: header.lines,
        documentTotal: header.documentTotal,
        updatedBy: user._id,
      },
      $inc: { __v: 1 },
    },
  ...
  return sanitizeLocalPurchase(updated.toObject());
}
...
async function submitOrResubmit(doc, user, { __v, isResubmit }) {
  ...
  const previousRejectionReason = isResubmit ? doc.rejectionReason : undefined;
  ...
  const populated = (await doc.populate('createdBy', 'name email')).toObject();

  const { notification } = await sendLocalPurchaseSubmitEmail(doc, populated, steps, {
    isResubmit,
    previousRejectionReason,
  });

  return {
    document: sanitizeLocalPurchase(populated),
    notification,
  };
}
...
export async function approveLocalPurchase(id, user, { comment, __v } = {}) {
  ...
  const doc = await loadLpForUpdate(id, user);
  const steps = await getApprovalSteps(DOC_TYPE);
  const step = getCurrentStep(steps, doc.currentApprovalStep) || resolveCurrentApprovalStep(steps, doc.toObject(), DOC_TYPE);
  if (!step) {
  ...
  const populatedUpdated = await LocalPurchase.findById(updated._id)
    .populate('createdBy', 'name email')
    .lean();
  const docId = updated._id.toString();
  const emailMeta = { documentType: DOC_TYPE, documentId: docId };

  if (after.isFinal) {
    const completionContext = await enrichLpEmailContextFromHistory(
      docId,
      buildLpEmailContext(populatedUpdated, {
    ...
    sendLocalPurchaseWorkflowEmail(
      ...
      completionContext,
      emailMeta,
      { document: populatedUpdated },
    ).catch((err) => {
      console.error('[email] local_purchase.completed failed:', err.message);
    });
  } else if (step.requiredPermission === 'lp.approve.pm') {
    const financeStep = getCurrentStep(steps, after.currentApprovalStep);
    const pmContext = buildLpEmailContext(populatedUpdated, {
      pmApproverName: user.name,
      approverName: user.name,
      comment,
    });
    sendLocalPurchaseWorkflowEmail(
      ...
      pmContext,
      emailMeta,
      { step: financeStep, document: populatedUpdated },
    ).catch((err) => {
      console.error('[email] local_purchase.pending_finance failed:', err.message);
    });
    sendLocalPurchaseWorkflowEmail(
      ...
      console.error('[email] local_purchase.pm_approved failed:', err.message);
    });
  }
...
export async function rejectLocalPurchase(id, user, { comment, __v } = {}) {
  ...
  const populatedUpdated = await LocalPurchase.findById(updated._id)
    .populate('createdBy', 'name email')
    .lean();
  const historyActorEmails = await resolveLpApprovalHistoryActorEmails(updated._id.toString());
  const creatorEmail = populatedUpdated.createdBy?.email?.toLowerCase?.();
  const extraTo = historyActorEmails.filter(
    (email) => email.toLowerCase() !== creatorEmail && email.toLowerCase() !== user.email?.toLowerCase?.(),
  );
  ...
  sendLocalPurchaseWorkflowEmail(
    'local_purchase.rejected',
    {
      ...buildLpEmailContext(populatedUpdated, {
        ...
        rejectingUserName: user.name,
        approverName: user.name,
        extraTo,
      }),
    },
    { documentType: DOC_TYPE, documentId: updated._id.toString() },
    { step, document: populatedUpdated },
  ).catch((err) => {
    console.error('[email] local_purchase.rejected failed:', err.message);
  });
...
export async function cancelLocalPurchase(id, user, { comment, __v } = {}) {
  ...
  const populated = (await doc.populate('createdBy', 'name email')).toObject();
  const historyActorEmails = await resolveLpApprovalHistoryActorEmails(doc._id.toString());
  const creatorEmail = populated.createdBy?.email?.toLowerCase?.();
  ...
  sendLocalPurchaseWorkflowEmail(
    'local_purchase.cancelled',
    {
      ...buildLpEmailContext(populated, {
        ...
        extraTo: historyActorEmails.filter((email) => email.toLowerCase() !== creatorEmail),
      }),
    },
    { documentType: DOC_TYPE, documentId: doc._id.toString() },
    { document: populated },
  ).catch((err) => {
    console.error('[email] local_purchase.cancelled failed:', err.message);
  });

  return sanitizeLocalPurchase(populated);
}
