...
export function recalculateLpLines(lines = []) {
  return lines.map((line) => {
    ...
    return {
      ...
      uom: line.uom ? String(line.uom).trim() : undefined,
...
function normalizeHeader(data) {
  ...
  return {
    ...
    projectName: data.projectName ? String(data.projectName).trim() : undefined,
    vendorName: String(data.vendorName || '').trim(),
    vendorReference: data.vendorReference ? String(data.vendorReference).trim() : undefined,
...
async function buildListFilter(user, { searchParams, tab }) {
  ...
  const projectCode = searchParams.get('projectCode') || searchParams.get('project');
  if (projectCode) filter.projectCode = { $regex: projectCode, $options: 'i' };
  const vendorName = searchParams.get('vendorName') || searchParams.get('vendor');
