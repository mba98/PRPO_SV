...
function pickItemCodeField(row) {
  if (!row) return undefined;
  return row.ItemCode ?? row.ITEMCODE ?? row.itemCode;
}

/**
 * Highest item code in OITM for the given prefix (e.g. 125 → 1250000133).
 */
export async function getLastItemCodeFromHana(prefix) {
  const schema = getHanaCompanySchema();
  const sql = buildLastItemCodeSql(schema);
  const likePattern = `${String(prefix).trim()}%`;
  const rows = await withHanaConnection((connection) => connection.query(sql, [likePattern]));
  const raw = pickItemCodeField(rows?.[0]);
