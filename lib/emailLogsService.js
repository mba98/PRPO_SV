...
import { canViewEmailLogs } from '@/lib/visibilityFilters.js';
import { assertCanAccessDocument } from '@/lib/documentAccess.js';
import { redactSecretsFromText } from '@/lib/logRedaction.js';
...
function sanitizeLog(row) {
  return {
    ...
    to: row.to || [],
    cc: row.cc || [],
    subject: redactSecretsFromText(row.subject),
    body: redactSecretsFromText(row.body),
    relatedDocumentType: row.relatedDocumentType,
    relatedDocumentId: row.relatedDocumentId?.toString() || null,
    emailStatus: row.emailStatus,
    sentAt: row.sentAt,
    errorMessage: row.errorMessage ? redactSecretsFromText(row.errorMessage) : null,
  };
}
...
export async function listEmailLogs(
  ...
  const hasDocumentScope = Boolean(relatedDocumentType && relatedDocumentId);

  if (hasDocumentScope) {
    await assertCanAccessDocument(user, relatedDocumentType, relatedDocumentId);
  } else if (!canViewEmailLogs(user)) {
    forbidden();
  }
  ...
  if (q) {
    ...
    filter.$or = [{ subject: regex }, { eventKey: regex }, { errorMessage: regex }];
  }
