...
export function userCanCreateApriInSap(user, apri) {
  ...
  return isApriReadyForSapCreation(apri.status);
}
...
export async function getApReserveInvoiceById(id, user) {
  return perfAsync(`getApReserveInvoiceById ${id}`, async () => {
    ...
    const permissions = getEffectivePermissions(user);
    const canEdit = userCanEditApriQuantities(user, apri);
    const canCreateInSap = userCanCreateApriInSap(user, apri);
    const canRetrySap = userCanRetryApriSap(user, apri);
