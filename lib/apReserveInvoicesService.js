...
import { sumApriDocumentTotal, sumLineQuantities } from '@/lib/documentTotals.js';
import {
  APRI_STATUS,
  apriStatusInQuery,
  apriStatusesEqual,
...
export function sanitizeApri(doc) {
  ...
  const documentTotal = sumApriDocumentTotal(lines);
  const totalQuantity = sumLineQuantities(lines);
  return {
    id,
...
async function buildApriDetailPayload(apriDoc, user, steps) {
  ...
  const totalQuantity = sumLineQuantities(enrichedLines);
  const documentTotal = sumApriDocumentTotal(enrichedLines);
  ...
  const canResubmit = userCanResubmitApri(user, enrichedApri);
  const canViewFinancials = userCanViewApriFinancials(user);
  ...
  const sanitizedDocument = canViewFinancials
    ? { ...document, canViewFinancials: true, documentTotal }
    : sanitizeApriFinancialFields(document, false);
...
export async function resubmitApri(id, user, { __v } = {}) {
  const apri = await loadApriForUpdate(id);
  if (!userCanResubmitApri(user, apri.toObject())) {
    const err = new Error('Forbidden');
    err.code = 'FORBIDDEN';
    throw err;
  }
  if (!apriStatusesEqual(apri.status, APRI_STATUS.WAREHOUSE_REJECTED)) {
    const err = new Error('Only warehouse-rejected APRI records can be resubmitted');
  ...
  await logApprovalHistory({
    ...
    previousStatus,
    newStatus: apri.status,
    comment: 'Procurement resubmitted after quantity correction',
  });
  ...
  notifyWorkflowEmailSafe(
    ...
    { documentType: DOC_TYPE, documentId: apri._id.toString() },
  );

  invalidateApriWorkflowCaches();

  const refreshed = await APReserveInvoice.findById(id).lean();
  const { document } = await buildApriDetailPayload(refreshed, user, steps);
  return document;
}
