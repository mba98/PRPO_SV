...
export function userCanCreateApriInSap(user, apri) {
  ...
  if (apri.sapAPDocEntry || apri.sapAPDocNum) return false;
  if (isApriSapInProgress(apri.status) || isApriCreatedInSap(apri.status)) return false;
  if (!isApriReadyForSapCreation(apri.status)) return false;
  return apriSavedQuantitiesAreValid(apri.lines);
}
...
export function userCanEditApriQuantities(user, apri) {
  ...
  if (!permissions.includes('view.all') && !userIsApriOwner(user, apri)) {
    return false;
  }
  if (apri.sapAPDocEntry) return false;
...
async function buildApriDetailPayload(apriDoc, user, steps) {
  ...
  const canCreateInSap = userCanCreateApriInSap(user, enrichedApri);
