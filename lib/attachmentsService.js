...
/**
 * Strip directory parts and reserved characters from a user-provided file name.
 */
...
export async function completeUpload(user, params) {
  ...
  await logApprovalHistory({
    ...
    comment: `Uploaded file: ${doc.fileName}`,
  });
...
export async function listAttachments(documentType, documentId, user, options = {}) {
  ...
  const rows = await Attachment.find({ documentType, documentId })
    .sort({ uploadedAt: -1 })
