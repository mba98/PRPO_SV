...
import { PR_SORT_FIELDS, resolveSortField } from '@/lib/listQuery.js';
...
import { canShowCreatePoAction } from '@/lib/poPermissions.js';
import { buildApprovalActionResult } from '@/lib/approvalActionResponse.js';
import { PR_LIST_SELECT, lineCount } from '@/lib/listFields.js';
import { perfAsync } from '@/lib/perfLog.js';
import { enforcePrLinesUom } from '@/lib/lineUomEnforcement.js';
import { sumPrDocumentTotal } from '@/lib/documentTotals.js';
...
function normalizeLines(lines = []) {
  return lines.map((line) => {
    ...
    const uomCode = (line.uomCode || line.uom || '').trim() || undefined;
    return {
      ...line,
      uomCode,
...
export function sanitizePr(doc) {
  ...
  return {
    ...
    lines: doc.lines || [],
    documentTotal: sumPrDocumentTotal(doc.lines || []),
...
export function sanitizePrListItem(doc) {
  const base = sanitizePr(doc);
  if (!base) return null;
...
export async function buildListFilter(user, params) {
  ...
  if (tab === 'my') {
    ...
    if (!permissions.includes('view.all') && !permissions.includes('pr.create')) {
      filter.requester = user._id;
    }
  } else if (tab === 'sap') {
  ...
  if (q) {
    const qFilter = {
      $or: [
        { portalPRNumber: { $regex: q, $options: 'i' } },
        { sapPRDocNum: { $regex: q, $options: 'i' } },
...
export async function listPurchaseRequests(user, { page, limit, sort, order, searchParams }) {
  return perfAsync('listPurchaseRequests', async () => {
    await connectDB();
    const tab = searchParams.get('tab') || 'my';
    const filter = await buildListFilter(user, { searchParams, tab });
    const sortDir = order === 'asc' ? 1 : -1;
    const sortField = resolveSortField(sort, PR_SORT_FIELDS);
    ...
    return {
      items: rows.map(sanitizePrListItem),
      pagination: buildPagination(page, limit, total),
    };
  });
}

export async function fetchPurchaseRequestsForExport(user, { searchParams, sort, order, limit }) {
  await connectDB();
  const tab = searchParams.get('tab') || 'my';
  const filter = await buildListFilter(user, { searchParams, tab });
  const sortDir = order === 'asc' ? 1 : -1;
...
export async function getPurchaseRequestById(id, user) {
  return perfAsync(`getPurchaseRequestById ${id}`, async () => {
    await connectDB();
    ...
    const [workflowSteps, relatedOrders, matrixSteps, approvalHistory] = await Promise.all([
      loadPrWorkflow(pr, user),
      PurchaseOrder.find({ relatedPRId: id })
        .select('portalPONumber status sapPODocNum sapPODocEntry vendor')
        .lean(),
      getApprovalSteps('PR'),
      getApprovalHistory('PR', id),
    ]);
    const poMeta = enrichPrForPoList(pr, relatedOrders);
    ...
    const canEdit = canEditPurchaseRequest(user, pr);
    return {
      ...sanitized,
      ...poMeta,
      workflowSteps,
      ...approvalAccess,
      canEdit,
      canResubmit: canResubmitPurchaseRequest(user, pr),
      rejectionReason: rejection?.reason || null,
      rejectionStepName: rejection?.stepName || null,
      rejectedByName: rejection?.rejectedBy || null,
      canCreatePo: canShowCreatePoAction(user, pr, { poReady: poMeta.poReady }),
      canRetrySap:
        isDocumentEligibleForSapRetry(pr, 'PR') &&
        canUserRetrySapDocument({
          user,
          documentType: 'PR',
          document: pr,
...
export async function createPurchaseRequest(data, user) {
  ...
  header.lines = await enforcePrLinesUom(header.lines || []);
...
export async function updatePurchaseRequest(id, data, user) {
  ...
  const denialMessage = getPrEditForbiddenMessage(user, pr.toObject());
  if (denialMessage) {
    const err = new Error(denialMessage);
    err.code = denialMessage.includes('permission') ? 'FORBIDDEN' : 'INVALID_STATUS';
    throw err;
  }
  ...
  if (header.lines != null) {
    header.lines = await enforcePrLinesUom(header.lines, pr.lines);
  }
  if (header.requiredDate != null) pr.requiredDate = header.requiredDate;
  if (header.documentDate != null) pr.documentDate = header.documentDate;
  if (header.dueDate != null) pr.dueDate = header.dueDate;
  if (header.postingDate != null) pr.postingDate = header.postingDate;
  if (header.project != null) pr.project = header.project;
  if (header.department != null) pr.department = header.department;
  if (header.warehouse != null) pr.warehouse = header.warehouse;
  if (header.remarks != null) pr.remarks = header.remarks;
  if (header.lines != null) {
    pr.lines = header.lines;
    pr.markModified('lines');
  }
  await pr.save();
  ...
  await logApprovalHistory({
    ...
    newStatus: pr.status,
    comment: 'Purchase request updated',
  });

  return getPurchaseRequestById(id, user);
}

function assertProcurementCanSubmitPr(user) {
  if (!userCanProcurementEditPr(user)) {
    const err = new Error('Only Procurement can submit or resubmit purchase requests');
    err.code = 'FORBIDDEN';
    throw err;
  }
}
...
export async function submitPurchaseRequest(id, user, { __v } = {}) {
  await connectDB();
  const pr = await loadPrForUpdate(id, user);
  assertProcurementCanSubmitPr(user);
  if (!isEditablePrStatus(pr.status)) {
    const err = new Error('Only draft or rejected purchase requests can be submitted');
    err.code = 'INVALID_STATUS';
    throw err;
  }
  if (pr.sapPRDocEntry) {
    const err = new Error('Purchase request cannot be resubmitted after SAP creation');
    err.code = 'INVALID_STATUS';
    throw err;
  }
  if (normalizePrStatus(pr.status) === 'Creating in SAP') {
    const err = new Error('Purchase request cannot be submitted while SAP creation is in progress');
    err.code = 'INVALID_STATUS';
    throw err;
  }
  ...
  const isResubmit = isResubmittablePrStatus(pr.status);
  ...
  await logApprovalHistory({
    documentType: 'PR',
    documentId: pr._id,
    stepName: isResubmit ? 'Resubmit' : step?.stepName || 'Submit',
  ...
  notifyWorkflowEmailSafe(
    ...
    { documentType: 'PR', documentId: pr._id.toString() },
  );
...
export async function approvePurchaseRequest(id, user, { comment, __v } = {}) {
  ...
  assertApprovalVersionMatches(pr, __v);
  ...
  if (after.isFinal) {
    notifyWorkflowEmailSafe(
      'pr.pm.approved',
      {
        ...buildPrEmailContext(updated),
        status: 'Approved — SAP creation has started',
        documentTypeLabel: 'Purchase Request',
      },
      { documentType: 'PR', documentId: updated._id.toString() },
    );

    const sapResult = await createSapPurchaseRequest(updated._id.toString(), user);
    const refreshed = await PurchaseRequest.findById(id).lean();
    return buildApprovalActionResult('PR', sanitizePr(refreshed), user, {
  ...
  return buildApprovalActionResult('PR', sanitizePr(updated.toObject()), user, {
    message: 'Approved successfully',
    sapResult: null,
  });
}
...
export async function rejectPurchaseRequest(id, user, { comment, __v } = {}) {
  ...
  const previousStatus = pr.status;
  const rejectedStatus = rejectedStatusForDocumentType('PR');
  ...
  notifyWorkflowEmailSafe(
    'pr.rejected',
    {
      ...
      rejectingStep: step?.stepName || null,
      rejectingStepName: step?.stepName || null,
      rejectingUserName: user.name || user.username || user.email,
...
/** @deprecated Use listPrsReadyForPo from purchaseOrdersService (approved-for-po API). */
export async function listApprovedForPo(user, { page, limit }) {
  const { listPrsReadyForPo } = await import('@/lib/purchaseOrdersService.js');
  return listPrsReadyForPo(user, { page, limit });
}
