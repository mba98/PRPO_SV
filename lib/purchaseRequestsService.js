...
function userCanActAsRequesterOrAdmin(user, pr) {
  const permissions = getEffectivePermissions(user);
  if (permissions.includes('view.all') || permissions.includes('admin.settings')) {
    return;
  }
...
export async function submitPurchaseRequest(id, user, { __v } = {}) {
  ...
  if (isResubmit) {
    if (!isPrRequester(user, pr.toObject())) {
      const err = new Error('Only the requester can resubmit this purchase request');
      err.code = 'FORBIDDEN';
      throw err;
    }
  } else {
    userCanActAsRequesterOrAdmin(user, pr);
  }
