...
function canAccessApri(_user, _apri, permissions) {
  return (
    permissions.includes('apinvoice.create') || permissions.includes('view.all')
  );
}

function canAccess(documentType, user, document) {
  ...
  if (documentType === 'APRI') return canAccessApri(user, document, permissions);
  return false;
}
...
export async function assertCanAccessDocument(user, documentType, documentId) {
  ...
  if (!canAccess(documentType, user, document)) {
    throw forbidden();
  }
  return document;
}
