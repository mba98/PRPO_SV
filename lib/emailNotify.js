...
/**
 * Resolve TO/CC for an event: EmailGroup override first, then configured fallbacks.
 */
export async function resolveEventRecipients(eventKey, context = {}) {
  ...
  if (context.requesterEmail) {
    const fallback = EVENT_FALLBACK_RECIPIENTS[eventKey];
    if (fallback?.useRequester || !group?.isActive) {
  ...
  const uniqueCc = [...new Set(cc.filter(Boolean).filter((e) => !uniqueTo.includes(e)))];
