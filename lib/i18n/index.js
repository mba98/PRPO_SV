...
import { APRI_STATUS_LABELS, apriStatusLabel, normalizeApriStatus } from '@/lib/apriStatus.js';
import { LP_STATUS_LABELS_EN, LP_STATUS_LABELS_AR, normalizeLpStatus } from '@/lib/localPurchaseStatus.js';
...
function normalizeStatusKey(status) {
  ...
  const apriNorm = normalizeApriStatus(status);
  if (APRI_STATUS_LABELS[apriNorm]) return APRI_STATUS_LABELS[apriNorm];
  const lpNorm = normalizeLpStatus(status);
  if (LP_STATUS_LABELS_EN[lpNorm]) return LP_STATUS_LABELS_EN[lpNorm];
...
export function statusLabel(status, locale = DEFAULT_LOCALE) {
  ...
  return (
    ...
    APRI_STATUS_LABELS[normalizeApriStatus(status)] ||
    (locale === 'ar' ? LP_STATUS_LABELS_AR : LP_STATUS_LABELS_EN)[normalizeLpStatus(status)] ||
    apriStatusLabel(status) ||
    status
  );
}
