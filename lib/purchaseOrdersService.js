...
import { assertPoDocCurrencyAndRateAllowedForVendor } from '@/lib/sap/vendorCurrencies.js';
...
import { enforcePoLinesUom } from '@/lib/lineUomEnforcement.js';
import { sumPoDocumentTotal } from '@/lib/documentTotals.js';
import {
...
export function sanitizePo(doc) {
  ...
  return {
    ...
    lines: doc.lines || [],
    documentTotal: sumPoDocumentTotal(doc.lines || []),
...
export async function updatePurchaseOrder(id, data, user) {
  ...
  if (data.docCurrency != null || data.vendor != null || data.documentDate != null) {
    const vendorCode = (data.vendor ?? po.vendor ?? '').trim();
    const currencyToValidate = data.docCurrency ?? po.docCurrency;
    const documentDate = parsePoDate(data.documentDate) ?? po.documentDate;
    if (vendorCode && currencyToValidate) {
      try {
        const { currency, docRate } = await assertPoDocCurrencyAndRateAllowedForVendor(
          vendorCode,
          currencyToValidate,
          documentDate,
        );
        ...
        const validationErr = new Error(
          err.message || 'The selected currency is not allowed for this Vendor.',
        );
  ...
  if (data.lines) {
    let prLinesById = new Map();
    if (po.relatedPRId) {
      const pr = await PurchaseRequest.findById(po.relatedPRId).lean();
      for (const pl of pr?.lines || []) {
        if (pl._id) prLinesById.set(pl._id.toString(), pl);
      }
    }
    const enforcedLines = await enforcePoLinesUom(
      data.lines,
      po.lines.map((l) => l.toObject?.() || l),
      prLinesById,
    );
    po.lines = normalizePoLines(enforcedLines).map((line, index) => {
...
export async function approvePurchaseOrder(id, user, { comment, __v } = {}) {
  ...
  const nextStep = getCurrentStep(steps, after.currentApprovalStep);
  const stepResolution = nextStep
    ? await resolveApprovalStepRecipientEmails(nextStep)
    : { emails: [] };
  notifyWorkflowEmailSafe(
    emailEvent,
    {
      ...
      requiredPermission: nextStep?.requiredPermission,
    },
    { documentType: DOC_TYPE, documentId: updated._id.toString() },
  );
