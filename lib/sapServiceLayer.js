...
/**
 * SAP Business One Service Layer currency rate for an exact date.
 * @see SBOBobService_GetCurrencyRate
 */
export async function getCurrencyRateFromServiceLayer(currency, date) {
  const payload = {
    Currency: currency,
    Date: date,
    RateDate: date,
  };
  const data = await post('/SBOBobService_GetCurrencyRate', payload);
  const raw = data?.Rate ?? data?.CurrencyRate ?? data?.rate ?? data?.value;
