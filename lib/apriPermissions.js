...
import { apriStatusInQuery, apriStatusesEqual } from '@/lib/apriStatus.js';
...
export async function userCanAccessApriWorkflow(user, steps = null) {
  const permissions = getEffectivePermissions(user);
  if (userIsApriProcurement(permissions)) return true;
  const matrix = steps
    ? [...new Set(steps.map((s) => s.requiredPermission).filter(Boolean))]
    : await getApriMatrixApprovalPermissions();
  return matrix.some((p) => permissions.includes(p));
}

export async function buildApriPendingApprovalFilter(user, steps = null) {
  const matrixSteps = steps || (await getApprovalSteps('APRI'));
  const permissions = getEffectivePermissions(user);

  if (permissions.includes('view.all')) {
    const statuses = matrixSteps.map((s) => pendingStatusForStep(s, 'APRI'));
    return { status: apriStatusInQuery(...statuses) };
  }
  ...
  for (const step of matrixSteps) {
    if (permissions.includes(step.requiredPermission)) {
      or.push({
        status: apriStatusInQuery(pendingStatusForStep(step, 'APRI')),
...
export async function buildApriListAccessFilter(user, steps = null) {
  const permissions = getEffectivePermissions(user);
  ...
  const pending = await buildApriPendingApprovalFilter(user, steps);
  if (pending.$or) {
    or.push(...pending.$or);
...
export async function userCanViewApriDocument(user, apri, steps = null) {
  const permissions = getEffectivePermissions(user);
  ...
  const matrixSteps = steps || (await getApprovalSteps('APRI'));
  const step = getCurrentStep(matrixSteps, apri.currentApprovalStep);
  if (step && permissions.includes(step.requiredPermission)) {
    return apriStatusesEqual(apri.status, pendingStatusForStep(step, 'APRI'));
  }
