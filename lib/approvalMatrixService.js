...
import { COMPLETION_POLICY } from '@/lib/approvalPolicies.js';
...
export function sanitizeApprovalMatrixStep(step) {
  ...
  return {
    ...
    completionPolicy: step.completionPolicy || COMPLETION_POLICY.ANY_ONE,
    minimumApprovalCount: step.minimumApprovalCount ?? null,
...
export async function createApprovalMatrixStep(data, user) {
  ...
  try {
    const step = await ApprovalMatrix.create({
      ...
      completionPolicy: data.completionPolicy || COMPLETION_POLICY.ANY_ONE,
      minimumApprovalCount: data.minimumApprovalCount ?? null,
...
export async function updateApprovalMatrixStep(id, data, user) {
  ...
  if (data.completionPolicy !== undefined) existing.completionPolicy = data.completionPolicy;
  if (data.minimumApprovalCount !== undefined) {
    existing.minimumApprovalCount = data.minimumApprovalCount;
  }
