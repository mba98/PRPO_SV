...
export function isValidPoStatusKey(status) {
  return PO_STATUS_VALUES.includes(normalizePoStatus(status));
}

/**
 * Resolve PO status for an approval matrix step (stable key only).
 * Permission and step name take precedence over matrix pendingStatus labels.
 */
export function pendingPoStatusForStep(step) {
  ...
  if (step.pendingStatus?.trim()) {
    const normalized = normalizePoStatus(step.pendingStatus.trim());
    if (isValidPoStatusKey(normalized)) {
      return normalized;
    }
  }
