...
import { resolvePoExchangeRateForDocument } from '@/lib/sap/exchangeRates.js';
...
function normalizeSubmittedPoLines(prObj, vendorCode, submittedLines = []) {
  ...
  for (const submitted of submittedLines) {
    ...
    normalized.push({
      ...
      quantity: qty,
      uom: prLine.uom,
      uomCode: resolveLineUomCode(prLine) || undefined,
      warehouseCode,
...
export async function createPortalPoFromPr(prId, user, body = {}) {
  ...
  let docRate;
  const documentDate =
    parsePoFormDate(body.documentDate) || pr.documentDate || headerDate;
  try {
    const config = await getVendorCurrencyConfig(vendorCode);
    const toValidate = body.docCurrency ?? config.defaultCurrency;
    const currencyResult = validatePoDocCurrencyForVendor(toValidate, config);
    if (!currencyResult.ok) {
      return {
        error: currencyResult.code || 'INVALID_CURRENCY',
        message: currencyResult.message || 'The selected currency is not allowed for this Vendor.',
      };
    }
    const resolved = await resolvePoExchangeRateForDocument({
      currency: currencyResult.currency,
      documentDate,
      localCurrency: config.companyLocalCurrency,
    });
    docCurrency = currencyResult.currency;
    docRate = resolved.docRate;
  } catch (err) {
    return {
      error: err.code || 'VENDOR_CURRENCY_CONFIG',
      message: err.message || 'Failed to load Vendor currencies from SAP.',
    };
  }
