 ...
 */
export async function createPortalPoFromPr(prId, user, { vendor, docCurrency: docCurrencyInput, docRate: docRateInput }) {
  await connectDB();
  ...
  const docCurrency = normalizePoDocCurrency(docCurrencyInput);
  const docRate = normalizePoDocRateForStorage(docCurrency, docRateInput);
