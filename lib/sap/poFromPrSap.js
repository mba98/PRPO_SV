...
import { getVendorCurrencyConfig, validatePoDocCurrencyAndRateForVendor } from '@/lib/sap/vendorCurrencies.js';
...
export async function createPortalPoFromPr(prId, user, body = {}) {
  ...
  try {
    ...
    const result = validatePoDocCurrencyAndRateForVendor(toValidate, body.docRate, config);
    if (!result.ok) {
      return {
        error: result.code || 'INVALID_CURRENCY',
        message: result.message || 'The selected currency is not allowed for this Vendor.',
      };
    }
    docCurrency = result.currency;
    docRate = result.docRate;
  } catch (err) {
    return {
