...
/** Client-safe SAP error { code, message } for API responses. */
export function formatSapErrorForClient(errorOrResponse) {
  const parsed = parseSapError(errorOrResponse);
  const codeNum = Number(parsed.code);
  return {
    code: Number.isFinite(codeNum) ? codeNum : parsed.code,
    message: parsed.message,
  };
}
