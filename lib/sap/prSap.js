...
async function logSapPrAttempt(args) {
  return writeSapIntegrationLog({
    documentType: 'PR',
    documentId: args.prId,
    action: 'CREATE_PR',
    requestPayload: args.logPayload,
    responsePayload: args.responsePayload ?? null,
    sapDocEntry: args.sapDocEntry,
    sapDocNum: args.sapDocNum,
    status: args.status,
...
async function failPrSapCreation({
  ...
  notifyWorkflowEmailSafe(
    'pr.sap.failed',
    {
      ...
      status: 'Failed to Create in SAP',
      sapErrorMessage: sanitizeSapErrorForEmail(message),
      documentId: prId?.toString?.() || prId,
    },
...
export async function createSapPurchaseRequest(prId, actingUser) {
  ...
  if (pr.sapPRDocEntry) {
    await logSapDuplicateGuard({
      ...
      documentId: prId,
      action: 'CREATE_PR',
      sapDocEntry: pr.sapPRDocEntry,
      sapDocNum: pr.sapPRDocNum,
      message: 'Duplicate guard: SAP PR already exists',
    });
    return {
      error: 'DUPLICATE_SAP',
