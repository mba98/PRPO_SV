...
async function getSapPrSettings() {
  const [branchDoc, departmentDoc, requesterDoc] = await Promise.all([
    SystemSettings.findOne({ key: 'branch_map' }).lean(),
    SystemSettings.findOne({ key: 'sap_department_map' }).lean(),
  ...
  return {
    ...
    departmentMap: departmentDoc?.value || {},
...
export async function createSapPurchaseRequest(prId, actingUser) {
  ...
  const { branchMap, departmentMap, defaultRequesterCode } = await getSapPrSettings();
  // Simplified PR flow: optional SAP reference fields (warehouse, project,
  // costing code, UDFs) are intentionally not sent. SAP applies its defaults.
