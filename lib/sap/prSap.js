...
export async function createSapPurchaseRequest(prId, user) {
  ...
  const requesterUser = pr.requester
    ? await User.findById(pr.requester).select('username sapRequesterCode email').lean()
    : null;
  ...
  const debugMeta = buildPrSapDebugMeta(pr, payload, {
