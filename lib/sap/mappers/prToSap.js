...
function mapPrLineToSap(line, pr, options = {}) {
  ...
  const lineDept = (line.uDepartment || '').trim();
  if (lineDept && !isInvalidSapOptionalCode(lineDept)) {
    docLine.U_Department = lineDept;
  }

  const projectCode = (line.projectCode || pr.project || '').trim();
  if (projectCode && !isInvalidSapOptionalCode(projectCode)) {
    docLine.ProjectCode = projectCode;
  }

  if (!options.omitCostingCode) {
    const costingCode = (line.costCenter || '').trim();
    if (costingCode && !isInvalidSapOptionalCode(costingCode)) {
      docLine.CostingCode = costingCode;
    }
  }
...
export function formatSapReferenceSummary(payload, debugMeta) {
  ...
  lines.forEach((line, i) => {
    const n = i + 1;
    parts.push(
      `Line ${n}: Item=${line.ItemCode || '—'}, Whs=${line.WarehouseCode || '—'}, Project=${line.ProjectCode || '—'}, CostCenter=${line.CostingCode || '—'}`,
    );
  });
  return parts.join('; ');
}
...
export function mapPrToSap(pr, options = {}) {
  ...
  const payload = {
    ...
    U_Department: resolveSapDepartmentUdf(pr.department, departmentMap),
