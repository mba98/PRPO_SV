...
export function buildStandaloneSapPoPayload(po, options = {}) {
  ...
  const localCurrency = normalizeLocalCurrency(options.localCurrency) || 'IQD';

  if (requiresPoDocRate(docCurrency, localCurrency)) {
    const storedRate = normalizePoDocRateForStorage(docCurrency, po.docRate, localCurrency);
    if (storedRate != null && storedRate > 0) {
      payload.DocRate = storedRate;
    }
  }
