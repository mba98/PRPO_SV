...
import { getCompanyLocalCurrency } from '@/lib/sapHana.js';
...
export async function createSapPurchaseOrder(poId, user) {
  ...
  const localCurrency = (await getCompanyLocalCurrency().catch(() => null)) || 'IQD';

  let poForSap = po.toObject();
  try {
    ...
    poForSap = { ...poForSap, docCurrency: currency, docRate };
    if (po.docCurrency !== currency || po.docRate !== docRate) {
      const update = { docCurrency: currency };
      if (docRate != null) {
        update.docRate = docRate;
        await PurchaseOrder.updateOne({ _id: poId }, { $set: update });
      } else {
        await PurchaseOrder.updateOne(
          { _id: poId },
          { $set: update, $unset: { docRate: '' } },
        );
      }
    }
  } catch (err) {
    return failPoSapCreation({
      ...
      user,
      payload: null,
      message: err.message || 'Failed to resolve exchange rate from SAP.',
      responsePayload: { code: err.code },
      errorCode: err.code || 'SAP_EXCHANGE_RATE_NOT_FOUND',
    });
  }

  const payload = mapPoToSapFromPortalRecord(poForSap, pr.toObject(), { localCurrency });
