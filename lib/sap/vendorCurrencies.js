...
export async function assertPoDocCurrencyAndRateAllowedForVendor(
  vendorCode,
  docCurrency,
  documentDate,
) {
  const config = await getVendorCurrencyConfig(vendorCode);
  const currencyResult = validatePoDocCurrencyForVendor(docCurrency, config);
  if (!currencyResult.ok) {
    const err = new Error(currencyResult.message);
    err.code = currencyResult.code;
    throw err;
  }
  try {
    const { docRate } = await resolvePoExchangeRateForDocument({
      currency: currencyResult.currency,
      documentDate,
      localCurrency: config?.companyLocalCurrency,
    });
    return { config, currency: currencyResult.currency, docRate };
  } catch (rateErr) {
    const err = new Error(rateErr.message || 'Failed to load exchange rate from SAP.');
    err.code = rateErr.code || 'SAP_EXCHANGE_RATE_NOT_FOUND';
    throw err;
  }
}
