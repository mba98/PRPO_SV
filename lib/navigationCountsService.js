...
import { APRI_STATUS, apriStatusInQuery, isApriFailedSap, normalizeApriStatus } from '@/lib/apriStatus.js';
...
export async function getNavigationCounts(user) {
  ...
  if (permissions.includes('apinvoice.create') || permissions.includes('view.all')) {
    ...
    const returnedFilter = {
      status: apriStatusInQuery(
        APRI_STATUS.WAREHOUSE_APPROVED,
        APRI_STATUS.WAREHOUSE_REJECTED,
      ),
    };
    if (!permissions.includes('view.all')) {
      returnedFilter.createdBy = user._id;
    }
    tasks.push(
      APReserveInvoice.countDocuments(returnedFilter).then((n) => {
        counts.returnedApri = n;
      }),
    );
  }
