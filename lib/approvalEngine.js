...
import { pendingLpStatusForStep, lpStatusesEqual } from '@/lib/localPurchaseStatus.js';
import { LP_STATUS } from '@/lib/localPurchaseStatus.js';
...
/**
 * Resolve the active matrix step for a document.
 * Local Purchase uses stored status as the source of truth when the step pointer is stale.
 */
export function resolveCurrentApprovalStep(steps, document, documentType = 'PR') {
  const docType = String(documentType || 'PR').toUpperCase();
  if (docType === 'LOCAL_PURCHASE' && document?.status && steps?.length) {
    const statusMatched = steps.find(
      (step) =>
        step.isActive !== false &&
        lpStatusesEqual(document.status, pendingLpStatusForStep(step, docType)),
    );
    if (statusMatched) return statusMatched;
  }
  return getCurrentStep(steps, document?.currentApprovalStep);
}
...
export function pendingStatusForStep(step, documentType = 'PR') {
  ...
  if (docType === 'LOCAL_PURCHASE') {
    return pendingLpStatusForStep(step);
  }
...
export function userCanApproveStep(user, step, documentType = 'PR') {
  if (!step?.requiredPermission) return false;
  const permissions = getEffectivePermissions(user);
  const docType = String(documentType || 'PR').toUpperCase();
  if (docType === 'LOCAL_PURCHASE') {
    return permissions.includes(step.requiredPermission);
  }
  return permissions.includes(step.requiredPermission);
}
