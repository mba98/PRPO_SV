...
/** Legacy fallback when matrix row has no pendingStatus (older seeds). */
const LEGACY_PERMISSION_STATUS = {
  ...
  'po.approve.om': 'Pending Operation Manager Approval',
...
export function pendingStatusForStep(step, _documentType = 'PR') {
  if (!step) return null;
