...
export function jsonSuccessNoStore(data, status = 200) {
...
export function handleServiceError(err) {
  ...
  if (err.code === 'VERSION_CONFLICT') {
    return jsonError(
      'The document was updated. Reload the latest version and try again.',
      'VERSION_CONFLICT',
      409,
    );
  }
  if (err.code === 'UOM_IMMUTABLE') {
    return jsonError(err.message, err.code, 400);
  }
  ...
  if (err.code === 'FORBIDDEN' || err.code === 'LOCAL_PURCHASE_FINANCE_APPROVAL_FORBIDDEN' || err.code === 'LOCAL_PURCHASE_PM_APPROVAL_FORBIDDEN') {
    return jsonError(err.message, err.code, 403);
  }
  if (
    ...
    err.code === 'DOC_RATE_REQUIRED' ||
    err.code === 'INVALID_DOC_RATE'
  ) {
    return jsonError(err.message, err.code, 400);
  }
