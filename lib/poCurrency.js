...
/** Foreign-currency PO documents require a positive SAP-resolved DocRate. */
...
/** Block PO submit when SAP exchange rate is loading or missing. */
export function getPoExchangeRateSubmitBlocker(
  header,
  localCurrency,
  { rateLoading = false, rateError = '' } = {},
  messages = {},
) {
  if (!requiresPoDocRate(header?.docCurrency, localCurrency)) return null;
  if (rateLoading) {
    return messages.loading || 'Loading exchange rate from SAP...';
  }
  if (rateError) return rateError;
  const raw = header?.docRate;
  if (raw === null || raw === '' || raw === undefined) {
    return (
      messages.missing ||
