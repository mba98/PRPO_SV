...
export function mapUomGroupRow(row) {
  const value = pickField(row, 'value', 'VALUE', 'UgpEntry');
  const label = pickField(row, 'label', 'LABEL', 'UgpName');
  const code = pickField(row, 'code', 'CODE', 'UgpCode');
  return {
...
export async function searchSapUomGroups(query, limit = 100) {
  ...
  return filterRows(mapped, query, ['label', 'code']).slice(0, limit);
}
