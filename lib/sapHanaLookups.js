...
export function mapCompanyRow(row) {
  return mapValueLabelRow(row, {
    valueKeys: ['Company', 'U_Company'],
    labelKeys: ['Company', 'U_Company'],
    codeKeys: ['Company', 'U_Company'],
  });
}
...
export async function searchSapAccounts(query, limit = 100) {
  ...
  const mapped = (rows || []).map(mapAccountRow).filter((r) => r.value);
  return filterRows(mapped, query, ['label', 'code', 'value']).slice(0, limit);
}

export async function searchSapCompanies(query, limit = 100) {
  const rows = await listCompanyValues(limit);
  const mapped = (rows || []).map(mapCompanyRow).filter((r) => r.value);
  return filterRows(mapped, query, ['label', 'code', 'value']).slice(0, limit);
}
