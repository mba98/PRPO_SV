...
/**
 * Company lookup for OITM.U_Company.
 * Primary source: UFD1 valid values (code + description). UFD1 is not guaranteed
 * to have entries, so values actually stored in OITM.U_Company are merged in.
 * Deduplicated by code; when no description exists, code is used as name.
 */
export async function searchSapCompanies(query, limit = 20) {
  const { validValues, usedValues } = await listCompanyValues(query, limit);
  const merged = new Map();

  for (const row of validValues || []) {
    const mapped = mapCompanyRow(row);
    if (!isValidLookupResult(mapped)) continue;
    const normalized = normalizeLookupResult(mapped);
  ...
  return [...merged.values()]
    .map((row) => {
      ...
      return { ...row, label: name, name };
    })
    .slice(0, Math.min(Math.max(1, Number(limit) || 20), 50));
}
