...
/**
 * True when every matrix approval step is finished (approved, rejected-to-procurement, or SAP-bound).
 * APRI uses currentApprovalStep=0 after warehouse approval — do not treat 0 as "before step 1".
 */
function isDocumentPastAllApprovals(doc, documentType) {
  if (documentType === 'PR') {
    if (resolveSapDocEntry(doc, documentType)) return true;
    const s = doc.status;
    return (
      s === 'Approved' ||
      s === 'Creating in SAP' ||
      s === 'Created in SAP' ||
      s === 'Failed to Create in SAP' ||
      s === 'Partially Ordered' ||
      s === 'Fully Ordered' ||
      (typeof s === 'string' && s.includes('Ordered'))
    );
  }
  if (documentType === 'PO') {
    return isPoApprovedOrSapStatus(doc.status) || Boolean(resolveSapDocEntry(doc, documentType));
  }
  if (documentType === 'APRI') {
    if (resolveSapDocEntry(doc, documentType)) return true;
    const norm = normalizeApriStatus(doc.status);
    return [
      APRI_STATUS.WAREHOUSE_APPROVED,
      APRI_STATUS.CREATING_IN_SAP,
      APRI_STATUS.CREATED_IN_SAP,
      APRI_STATUS.FAILED_SAP,
    ].includes(norm);
  }
  if (documentType === 'LOCAL_PURCHASE') {
    return lpStatusesEqual(doc.status, LP_STATUS.COMPLETED);
  }
  return false;
}
...
export function buildApprovalWorkflowSteps(steps, doc, documentType, user) {
  ...
  const allApprovalsComplete = isDocumentPastAllApprovals(doc, documentType);
  ...
  return steps.map((step) => {
    ...
    if (rejected) {
      ...
      state = 'completed';
    } else if (draft || (currentOrder > 0 && currentOrder < step.stepOrder)) {
      state = 'pending';
