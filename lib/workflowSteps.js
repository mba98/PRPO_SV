...
export function buildCreatedWorkflowStep(doc) {
  ...
  } else if (doc.status === 'Rejected' && !doc.currentApprovalStep) {
    state = 'rejected';
  }
  return {
    kind: 'created',
...
/** APRI: Created → SAP APRI Created (no approval matrix steps). */
export function buildApriWorkflow(apri) {
  let createdState = 'completed';
  if (apri.status === 'Ready for AP Reserve Invoice') {
  ...
  const sapStep = buildSapWorkflowStep(apri, 0, 'APRI');
  return [created, sapStep];
}

export async function loadApriWorkflow(apri) {
  return buildApriWorkflow(apri);
}
