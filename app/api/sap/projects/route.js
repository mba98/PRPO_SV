...
import { searchSapProjects } from '@/lib/sapLookups.js';
...
async function getHandler(request) {
  try {
    ...
    const items = await searchSapProjects(query, limit);
    return jsonSuccessCached(items);
  } catch (err) {
    return sapLookupFailureResponse('sap/projects', err, 'Failed to load projects');
  }
}
